...
const ChallanCheck = () => {
  ...
  const [challanNo, setChallanNo] = useState("test");

  const getOrderDetails = () => {
    setLoader(true);
    const apiUrl = BASE_URL + "api/v1/public/check-challan/" + challanNo;
    axios
      .get(apiUrl)
      .then((res) => {
    ...
    <>
      ...
      {loader ? (
        ...
        <>
          ...
          {isEmpty && <h5 className="mt-5 text-danger ms-1">Enter a valid challan no.</h5>}
        </>
      )}
    </>
  );
