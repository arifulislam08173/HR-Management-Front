...
const MaterialReport = ({ token }) => {
  ...
  const getClosingQty = (item) => {
    let qty =
      item.total_open_stock_qty +
      item.total_finished_qty +
      item.total_purchase_qty +
      item.total_sales_return_qty -
      item.total_sales_qty -
      item.total_purchase_return_qty;
        ...
        if (response.data.status) {
          const data = response.data.data.map((stock, index) => {
            return {
              ...
              Opening: stock.total_open_stock_qty,
              Production: stock.total_finished_qty,
              Sales: stock.total_sales_qty,
              "Sales Return": stock.total_sales_return_qty,
              Purchase: stock.total_purchase_qty,
              "Purchase Return": stock.total_purchase_return_qty,
              Closing: getClosingQty(stock),
            };
          });
          console.log(data);
              ...
              <tbody>
                {stocks?.map((stock, index) => (
                  <tr key={index}>
                    ...
                    <td>{stock.total_open_stock_qty}</td>
                    <td>{stock.total_finished_qty}</td>
                    <td>{stock.total_sales_qty}</td>
                    <td>{stock.total_sales_return_qty}</td>
                    <td>{stock.total_purchase_qty}</td>
                    <td>{stock.total_purchase_return_qty}</td>
                    <td>{getClosingQty(stock)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
export default connect(mapStateToProps)(MaterialReport);
