...
const productionReportMaterials = ({ token }) => {
        ...
        if (response.data.status) {
          const data = response.data.data.map((stock, index) => {
            return {
              ...
              Consumption: stock.totalConsumption.toFixed(2),
              ...
              <tbody>
                {stocks?.map((stock, index) => (
                  <tr key={index}>
                    ...
                    <td>{stock.totalConsumption.toFixed(2)}</td>                     
...
export default connect(mapStateToProps)(productionReportMaterials);
