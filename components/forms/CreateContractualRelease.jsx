...
const CreateContractualRelease = ({ token, userBranch }) => {
  ...
  const [driver_name, setDriverName] = useState(null);
  const [driver_mobile, setDriverMobile] = useState(null);
  ...
  const searchProduct = async (e) => {
    ...
    const newTimer = setTimeout(() => {
      if (e == "") {
        ...
        setProducts([]);
        const apiProduct =
          BASE_URL + "api/v1/product-search?type=2&keyword=" + e + "&sales=" + 1;
        axios
          .get(apiProduct, {
            headers: { Authorization: "Bearer " + token },
          })
...
export default connect(mapStateToProps)(CreateContractualRelease);
